...
function unitPoint(p) {
  ...
  var result  = vec4(p[0], p[1], p[2], p[3]);
...
/**
 * DOESN'T WORK - PROBLEMS WITH JS RECURSION.
 *
...
/**
 * RECURSIVE VERSION DOESN'T WORK - SEE BELOW FOR ITERATIVE VERSION.
 *
 ...
 * Generate a spherical polyhedron with vertices at
 * unit radius, centered at the origin.
 * This function generates the polyhedron by
 * subdividing the faces of an octohedron; if
 * qEdges is 0, it generates the unit radius octohedron.
 * The generated vertices are appended to points as triangles,
 * and the number of vertices generated is returned.
 * The number of vertices appended to points and
 * the number of edges around the circumference are as follows:
 *
 *   qEdges   points       edges around circumference
 *     1         24            4
 *     2         96            8
 *     3        216           12
 *     4        384           16
 ...
 *     k         24 * k^2      4 * k
 *
 * Note that with smooth shading, qEdges = 16 is probably enough
 * to approximate a sphere for most purposes.
 *
 * @author  J. Andrew Whitford Holey
 * @version February 10, 2016
 * @param   qEdges  the number of edges in a quarter circumference
 * @param   points  the array of points to append the triangles to
 * @return  the number of vertices appended to points (24 * qEdges^2)
 * @throws  exception if qEdges < 1
 */
function spherichedron(qEdges, points) {

  if (qEdges < 1) throw "spherichedron: fewer than 1 edge per quarter"

  var xVals = [];
  var yVals = [];
  var zVals = [];
  var zSlice = Math.PI / (2 * qEdges); // z-arc in one quarter

  // generate coordinates for points in upper right front octant
  for (k = 0; k <= qEdges; k++) {
    xVals.push([]);
    yVals.push([]);
    var zAngle = k * zSlice;
    zVals.push(Math.sin(zAngle));
    var cLimit = qEdges - k;
    if (cLimit > 0) {
      var zRad   = Math.cos(zAngle);
      var cSlice = Math.PI / (2 * cLimit);
      for (i = 0; i <= cLimit; i++) {
        var cAngle = i * cSlice;
        xVals[k].push(zRad * Math.cos(cAngle));
        yVals[k].push(zRad * Math.sin(cAngle));
      }
    } else { // cLimit == 0; k = qEdges; zRad = 0.0
      xVals[k].push(0.0);
      yVals[k].push(0.0);
    }
  }
  ...
  for (k = 0; k < qEdges; k++) {
    ...
    // equatorial triangles in row
    for (i = 0; i < cLimit; i++) {
      // upper right front
      points.push(vec4(xVals[k][i],   yVals[k][i],   zVals[k],   1.0));
      points.push(vec4(xVals[k][i+1], yVals[k][i+1], zVals[k],   1.0));
      points.push(vec4(xVals[k+1][i], yVals[k+1][i], zVals[k+1], 1.0));

      // upper left front
      points.push(vec4(-yVals[k][i],   xVals[k][i],   zVals[k],   1.0));
      points.push(vec4(-yVals[k][i+1], xVals[k][i+1], zVals[k],   1.0));
      points.push(vec4(-yVals[k+1][i], xVals[k+1][i], zVals[k+1], 1.0));
    ...
    // polar triangles in row
    for (i = 0; i < cLimit-1; i++) {
      // upper right front
      points.push(vec4(xVals[k+1][i],   yVals[k+1][i],   zVals[k+1], 1.0));
      points.push(vec4(xVals[k][i+1],   yVals[k][i+1],   zVals[k],   1.0));
      points.push(vec4(xVals[k+1][i+1], yVals[k+1][i+1], zVals[k+1], 1.0));

      // upper left front
      points.push(vec4(-yVals[k+1][i],   xVals[k+1][i],   zVals[k+1], 1.0));
      points.push(vec4(-yVals[k][i+1],   xVals[k][i+1],   zVals[k],   1.0));
      points.push(vec4(-yVals[k+1][i+1], xVals[k+1][i+1], zVals[k+1], 1.0));
